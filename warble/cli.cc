...
void CLI::Warble(std::string user, std::string text, std::string parent) {
	...
	*(req.mutable_payload()) = tx_payload;
	...
	if (s.ok()) {
		...
		std::cout << "Could not post the warble. Please make sure you are signed up and logged in.";
